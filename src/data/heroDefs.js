...
(function () {
  ...
  const HERO_DEFS = [
    ...
    // —— 擴充武將（酒館招募） ——
    // 蜀
    hero('fazheng', '法正', 'shu', 4, 45, 82, 95, '奇謀佐世', '進攻時大幅削弱敵軍防禦',
      [{ stat: 'enemyDefPct', value: -12, when: 'attacking' }],
      { type: 'recruit' }),
    hero('masu', '馬謖', 'shu', 3, 60, 78, 88, '談兵', '戰鬥中我軍傷亡降低',
      [{ stat: 'lossReductionPct', value: 8 }],
      { type: 'recruit' }),
    hero('guanping', '關平', 'shu', 3, 86, 78, 62, '將門虎子', '出戰步兵攻擊力提升',
      [{ stat: 'unitAtkPct', unit: 'infantry', value: 12 }],
      { type: 'recruit' }),
    hero('wangping', '王平', 'shu', 3, 80, 84, 68, '無當飛軍', '防守時防禦力提升',
      [{ stat: 'defPct', value: 12, when: 'defending' }],
      { type: 'recruit' }),
    hero('liaohua', '廖化', 'shu', 2, 78, 72, 60, '果敢', '全軍攻擊小幅提升',
      [{ stat: 'atkPct', value: 8 }],
      { type: 'recruit' }),
    hero('yanyan', '嚴顏', 'shu', 3, 82, 80, 66, '老當益壯', '所率部隊生命上限提升',
      [{ stat: 'hpPct', value: 12 }],
      { type: 'recruit' }),

    // 魏
    hero('xiahouyuan', '夏侯淵', 'wei', 4, 92, 83, 60, '神速', '騎兵攻擊力提升',
      [{ stat: 'unitAtkPct', unit: 'cavalry', value: 16 }],
      { type: 'recruit' }),
    hero('pangde', '龐德', 'wei', 4, 91, 80, 58, '抬櫬決死', '進攻時攻擊力大幅提升',
      [{ stat: 'atkPct', value: 14, when: 'attacking' }],
      { type: 'recruit' }),
    hero('yujin', '于禁', 'wei', 3, 82, 85, 66, '毅重', '防守時我軍傷亡降低',
      [{ stat: 'lossReductionPct', value: 10, when: 'defending' }],
      { type: 'recruit' }),
    hero('yuejin', '樂進', 'wei', 3, 85, 78, 60, '先登陷陣', '進攻時首輪傷害提升',
      [{ stat: 'firstStrikePct', value: 14, when: 'attacking' }],
      { type: 'recruit' }),
    hero('xunyu', '荀彧', 'wei', 5, 40, 90, 98, '王佐之才', '戰鬥中我軍傷亡降低且增益戰利',
      [{ stat: 'lossReductionPct', value: 10 }, { stat: 'lootBonusPct', value: 12 }],
      { type: 'recruit' }),
    hero('caopi', '曹丕', 'wei', 4, 72, 86, 84, '受禪', '全軍防禦力提升',
      [{ stat: 'defPct', value: 10 }],
      { type: 'recruit' }),
    hero('dengai', '鄧艾', 'wei', 4, 88, 90, 84, '偷渡陰平', '攻城器械攻擊力大幅提升',
      [{ stat: 'unitAtkPct', unit: 'siege', value: 18 }],
      { type: 'recruit' }),

    // 吳
    hero('sunce', '孫策', 'wu', 5, 92, 88, 68, '小霸王', '全軍攻擊力大幅提升',
      [{ stat: 'atkPct', value: 12 }],
      { type: 'recruit' }),
    hero('sunjian', '孫堅', 'wu', 4, 88, 84, 66, '江東猛虎', '進攻時攻擊力提升',
      [{ stat: 'atkPct', value: 12, when: 'attacking' }],
      { type: 'recruit' }),
    hero('chengpu', '程普', 'wu', 3, 80, 82, 70, '宿將', '防守時防禦力提升',
      [{ stat: 'defPct', value: 12, when: 'defending' }],
      { type: 'recruit' }),
    hero('handang', '韓當', 'wu', 3, 82, 76, 60, '弓馬嫻熟', '弓弩攻擊力提升',
      [{ stat: 'unitAtkPct', unit: 'ranged', value: 14 }],
      { type: 'recruit' }),
    hero('dingfeng', '丁奉', 'wu', 3, 84, 78, 64, '雪中奮短兵', '進攻時首輪傷害提升',
      [{ stat: 'firstStrikePct', value: 12, when: 'attacking' }],
      { type: 'recruit' }),
    hero('zhugejin', '諸葛瑾', 'wu', 3, 50, 80, 84, '和輯', '戰鬥中削弱敵軍攻擊',
      [{ stat: 'enemyAtkPct', value: -8 }],
      { type: 'recruit' })
  ];
