...
(function () {
   ...
   * @property {'story'|'explore'} type 取得方式：主線劇情解鎖，或酒館派遣探索取得。
   * @property {string} [missionId] type 為 'story' 時，解鎖此武將的關卡 id。
   * @property {string} [tileTag] type 為 'explore' 時，對應的探索據點 tag。
   */
  ...
  const HERO_DEFS = [
    ...
    hero('machao', '馬超', 'shu', 4, 95, 82, 60, '西涼鐵騎', '騎兵攻擊力提升', { type: 'explore', tileTag: 'xiliang' }),
    hero('huangzhong', '黃忠', 'shu', 4, 92, 79, 65, '百步穿楊', '弓騎兵/弩兵造成的傷害提升', { type: 'explore', tileTag: 'nanjun' }),
    hero('zhugeliang', '諸葛亮', 'shu', 5, 38, 92, 100, '奇謀', '戰鬥中我軍傷亡降低且獲得額外資源', { type: 'story', missionId: 'm4' }),
    hero('jiangwei', '姜維', 'shu', 3, 85, 80, 82, '繼志', '統率部隊在攻城戰中傷害提升', { type: 'explore', tileTag: 'tianshui' }),
    ...
    hero('liyan', '李嚴', 'shu', 2, 70, 68, 70, '治政', '主城資源產出小幅提升', { type: 'explore', tileTag: 'jiangzhou' }),
    ...
    hero('xiahoudun', '夏侯惇', 'wei', 4, 90, 82, 58, '拔矢', '受到致命傷害後短暫免疫死亡', { type: 'explore', tileTag: 'puyang' }),
    hero('zhangliao', '張遼', 'wei', 4, 91, 88, 68, '威震', '突襲敵軍時造成額外傷害', { type: 'explore', tileTag: 'hefei' }),
    hero('xuhuang', '徐晃', 'wei', 3, 87, 82, 64, '斷金', '對攻城器械造成額外傷害', { type: 'explore', tileTag: 'yangping' }),
    hero('dianwei', '典韋', 'wei', 3, 93, 70, 40, '惡來', '自身防禦力大幅提升', { type: 'story', missionId: 'm6' }),
    hero('xuchu', '許褚', 'wei', 3, 92, 68, 38, '虎癡', '所率部隊生命上限提升', { type: 'explore', tileTag: 'qiao' }),
    hero('guojia', '郭嘉', 'wei', 4, 40, 84, 95, '遺計', '戰前可預知敵軍配置並提升先攻', { type: 'story', missionId: 'm7' }),
    hero('zhangjunyi', '張郃', 'wei', 3, 86, 83, 63, '巧變', '地形不利時仍維持部隊戰力', { type: 'explore', tileTag: 'hanzhong' }),
    hero('caoren', '曹仁', 'wei', 2, 84, 78, 55, '堅守', '駐守城池時防禦力提升', { type: 'explore', tileTag: 'nanyang' }),
    ...
    hero('ganning', '甘寧', 'wu', 4, 90, 80, 60, '錦帆', '奇襲敵軍時速度與傷害提升', { type: 'explore', tileTag: 'jiangxia' }),
    hero('taishici', '太史慈', 'wu', 3, 89, 78, 58, '猛擊', '單挑戰鬥中攻擊力大幅提升', { type: 'explore', tileTag: 'shenting' }),
