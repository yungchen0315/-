...
(function () {
  ...
  const HERO_DEFS = [
    ...
    hero('simayi', '司馬懿', 'wei', 5, 70, 95, 97, '鷹視', '防守時大幅削弱敵軍攻擊力', { type: 'recruit' }),
    hero('xiahoudun', '夏侯惇', 'wei', 4, 90, 82, 58, '拔矢', '受到致命傷害後短暫免疫死亡', { type: 'recruit' }),
    hero('zhangliao', '張遼', 'wei', 4, 91, 88, 68, '威震', '突襲敵軍時造成額外傷害', { type: 'story', missionId: 'wei_m3' }),
    hero('xuhuang', '徐晃', 'wei', 3, 87, 82, 64, '斷金', '對攻城器械造成額外傷害', { type: 'recruit' }),
    hero('dianwei', '典韋', 'wei', 3, 93, 70, 40, '惡來', '自身防禦力大幅提升', { type: 'story', missionId: 'wei_m2' }),
    hero('xuchu', '許褚', 'wei', 3, 92, 68, 38, '虎癡', '所率部隊生命上限提升', { type: 'story', missionId: 'wei_m5' }),
    hero('guojia', '郭嘉', 'wei', 4, 40, 84, 95, '遺計', '戰前可預知敵軍配置並提升先攻', { type: 'story', missionId: 'wei_m4' }),
    hero('zhangjunyi', '張郃', 'wei', 3, 86, 83, 63, '巧變', '地形不利時仍維持部隊戰力', { type: 'story', missionId: 'wei_m6' }),
    hero('caoren', '曹仁', 'wei', 2, 84, 78, 55, '堅守', '駐守城池時防禦力提升', { type: 'recruit' }),

    hero('sunquan', '孫權', 'wu', 4, 68, 88, 80, '據江', '水域附近作戰時全軍戰力提升', { type: 'story', missionId: 'wu_m1' }),
    hero('zhouyu', '周瑜', 'wu', 5, 71, 94, 93, '火攻', '戰鬥開始對敵軍造成一波火焰傷害', { type: 'story', missionId: 'wu_m6' }),
    hero('luxun', '陸遜', 'wu', 5, 75, 92, 90, '連營', '反擊時造成的傷害大幅提升', { type: 'story', missionId: 'wu_m10' }),
    hero('ganning', '甘寧', 'wu', 4, 90, 80, 60, '錦帆', '奇襲敵軍時速度與傷害提升', { type: 'story', missionId: 'wu_m3' }),
    hero('taishici', '太史慈', 'wu', 3, 89, 78, 58, '猛擊', '單挑戰鬥中攻擊力大幅提升', { type: 'story', missionId: 'wu_m2' }),
    hero('lumeng', '呂蒙', 'wu', 4, 82, 85, 78, '奇襲荊州', '攻城戰中兵力損耗降低', { type: 'story', missionId: 'wu_m9' }),
